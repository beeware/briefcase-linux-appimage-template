...
2. Run ``cookiecutter`` on the template::
    ...
    $ cookiecutter https://github.com/beeware/briefcase-linux-appimage-template --checkout 3.6
...
3. `Download the Python Linux support package for x86_64`_, and extract it into
   the ``My Project.AppDir/usr`` directory generated by the template. This will
   give you a self-contained Python install. If installed correctly, there
   should be a ``My Project.AppDir/usr/bin/python3`` binary, a
   ``My Project.AppDir/usr/lib/python3.6`` directory, as well as some other
...
4. Add your code to the template, into the ``My Project.AppDir/usr/app``
   ...
   ``My Project.AppDir/usr/app_packages`` directory.
...
.. _Download the Python Linux support package for x86_64: https://briefcase-support.s3-us-west-2.amazonaws.com/python/3.6/linux/Python-3.6-linux-x86_64-support.b1.tar.gz
